    ...
    public class roll : ICommand
    {
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            foreach (Player p in nearbyPlayers)
            {
                ...
                {
                    Text = $"<b><color=#6a6964ff> <size=14>{Config.ServerName}</size></color><color=#fade04ff> {displayNameText}</color><color=#6a6964ff> /</color> <b><color=#200ae7ff>ROLL</color></b>: <color=#fade04ff>{message}</color></b>",
                    BottomBoundary = 950f,
                    RightBoundary = -1100,
                    LeftBoundary = -1100,
                    TargetX = -1100f,
